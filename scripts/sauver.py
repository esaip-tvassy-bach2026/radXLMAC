...
def remplir_colonnes(fichier_charge,colonnes,nb_radios,formater,ligne_base=2):
    """
    ...
    Sortie : fichier_charge(tpl) (le fichier modifié)
    """
    ...
    for rangee in feuille.iter_rows(min_row=ligne_base+1,max_col=feuille.max_column):
        mac_cellule=rangee[col_mac_physique]
        ...
        if mac_valeur:
            ...
            for idx_radio,col_idx in enumerate(col_radio):
                if idx_radio<len(les_increments):
                    ...
                    if formater==True:
                        rangee[col_idx].value=mac_radio_formatee
                    else:
                        rangee[col_idx].value=mac_radio
    return fichier_charge # On retourne le fichier modifié.
...
def sauvegarde_finale(classeur_modifie,nom_fichier):
    ...
    Entrée : classeur_modifie(), nom_fichier(str)
    Sortie : message_reussite(str)
    """
    assert isinstance(nom_fichier,str), "ERREUR FATALE : Les informations entrées en paramètres de cette fonction sont incorrects ou ne sont pas dans le bon format. Veuillez redémarrer compètement le programme et corriger votre saisie. Le programme doit s-arrêter ici."
