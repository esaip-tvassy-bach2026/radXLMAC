...
def cerveau_principal(former,fichier_source,feuille_cible,nom_col_mac_B,nbr_rad,nom_col_mac_RAD,ligne_base=2):
    ...
    params_colonnes=[fichier_charge,nbr_rad,nom_col_mac_B]+noms_colonnes_radio
    colonnes=charger_colonnes(*params_colonnes,ligne_depart=ligne_base)
    # Quatrième étape : on modifie le fichier aux bons endroits.
    fichier_modifie=remplir_colonnes(fichier_charge,colonnes,nbr_rad,former,ligne_base)
    # Cinquième étape : on enregistre le fichier modifié.
    classeur_modifie=fichier_modifie[0]
    sauvegarde_finale(classeur_modifie,fichier_new)
    # Message de fin.
